...
def test_host_history_converter_preserves_malformed_sidecar_for_safe_replay():
    """Malformed persisted sidecars remain marked so inbound replay uses the safe Host fallback."""
    host_payload = {
    ...
    assert json.loads(converted[0]["content"]) == host_payload
    assert converted[0][_AGUI_TOOL_RESULT_MODEL_CONTENT_KEY] == persisted[_AGUI_TOOL_RESULT_MODEL_CONTENT_KEY]
    replayed = agui_messages_to_agent_framework(converted)
    assert replayed[0].contents[0].result == "Safe Host summary"
